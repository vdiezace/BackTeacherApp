...
const getById = (userId) => {
  return db.query(
    "SELECT u.id AS user_id, DATE_FORMAT(subscribed, '%d/%m/%Y %H:%i'), DATE_FORMAT(unsubscribed, '%d/%m/%Y %H:%i'), first_name, last_name, username, email, password, role_id, title, description FROM users AS u INNER JOIN role AS r ON r.id = u.role_id WHERE u.id = ?",
    [userId]
  );
};
