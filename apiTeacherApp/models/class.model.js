...
const sqlClassesByStudentId =
  ...
  "DATE_FORMAT(c.creation, '%d/%m/%Y %H:%i') as creation_date, DATE_FORMAT(c.start_date,'%d/%m/%Y %H:%i') as start_date," +
  "c.start_hour, c.end_hour FROM classes as c JOIN teachers as t ON c.teachers_id = t.id JOIN students as s ON " +
  "c.students_id = s.id JOIN categories as cat ON t.category_id = cat.id JOIN users as u ON t.user_id = u.id WHERE s.id = ?";
